...
exports.createTodo = async (req, res) => {
	...
	try {
		...
		if (result) {
			...
			return res.json(
				jsonGenerator(StatusCode.CREATED, "Todo created Successfully"),
			);
		}
	} catch (e) {
		return res.json(
...
exports.getTodos = async (req, res) => {
	try {
		const list = await User.findById(req.userId).populate("todos").exec();

		return res.json(
			jsonGenerator(StatusCode.SUCCESS, "All todo list", list.todos),
		);
	} catch (e) {
		return res.json(
			jsonGenerator(
...
exports.removeTodo = async (req, res) => {
	...
	try {
		...
		if (result) {
			User.findByIdAndUpdate({ _id: req.userId }, { $pull: { todos: todoId } });
			return res.send(
