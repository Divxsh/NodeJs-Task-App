...
const Login = async (req, res) => {
	...
	if (!errors.isEmpty()) {
		return res.send(jsonGenerator(StatusCode.UNPROCESSABLE_ENTITY,"Checks failed"));
	} 
    else {
		...
		if (!searchUser) { // if not username found.
			return res.send(
				jsonGenerator(
			...
			const verified = bcrypt.compareSync(userPassword, searchUser.password);
